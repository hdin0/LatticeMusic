...
function move() {
  ...
  for (let i = 0; i < count; i++ ) {
    ...
    if ((listening)&& (tii%marker == marker-1)) {
      collectAndPlay( count );
    }
  }
}

function collectAndPlay( num, ){
...
function playLoop(vals, it) {
  if (it){ //if we're not on the first round, then clear what's in delwrite.
    for (let i=0; i<count; i++) {
      delwrite.i(0).disconnect( oscArr[i].o(0) );
    }
  }

  for (let i=0; i<count; i++){
    let v = vals[i];
    linArr[i].i(0).message([v]);
  }
  ...
  delread.o(0).connect( volScale.i(0));
  volScale.o(0).connect( lowpass.i(0));
  lowpass.o(0).connect(dac.i(0));
  lowpass.o(0).connect(dac.i(1));
}
