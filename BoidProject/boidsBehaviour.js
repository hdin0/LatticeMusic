  ...
  for (let i = 0; i < count; i++ ) {
    ...
    for (let tar = 0; tar < count; tar++ ) {
      ...
      if ((dist < eyeDist) && (theta < thetaLim)){
        cforceX += diff.x*(dist/eyeDist);
        cforceY += diff.y*(dist/eyeDist);
        cforceZ += diff.z*(dist/eyeDist);
...
// let a = new THREE.Vector3(4,3,0);
// let b = new THREE.Vector3(4,0,0);
// console.log(alignment(0.5,b.x,b.y,b.z,a,1));
...
function distanceToPeriodic( a, b, mod_length ) {
  ...
  let xdiff = Math.min( Math.abs(b.x-a.x), Math.abs(mod_length-b.x+a.x), Math.abs(mod_length-a.x+b.x) );
  let ydiff = Math.min( Math.abs(b.y-a.y), Math.abs(mod_length-b.y+a.y), Math.abs(mod_length-a.y+b.y) );
  let zdiff = Math.min( Math.abs(b.z-a.z), Math.abs(mod_length-b.z+a.z), Math.abs(mod_length-a.z+b.z) );
...
function vectorToPeriodic( a, b, mod_length) {
  ...
  let bb = new THREE.Vector3( b.x-a.x, b.y-a.y, b.z-a.z );
  bb.multiplyScalar( d/hypot3(bb.x, bb.y, bb.z ) );
  return bb;

}

//sepForce needs to be from 0 to 1.
function separation( sepForce, sfX, sfY, sfZ, currentPos, n) {
  let addv = new THREE.Vector3( sfX, sfY, sfZ );
  addv.multiplyScalar( sepForce/hypot3( sfX, sfY, sfZ ) );
  return addv;
}
...
//aliForce needs to be from 0 to 1.
function alignment( aliForce, afX, afY, afZ, avec ) {
  // we want to only use the parallel component.
  // console.log(avec);
  let b = new THREE.Vector3( afX, afY, afZ );
  let a = new THREE.Vector3( avec.x, avec.y, avec.z );
  let aperp = a;
  if (hypot3(b.x,b.y,b.z)){
    aperp.sub(b.multiplyScalar((a.dot(b))/Math.pow(hypot3(b.x,b.y,b.z),2)));
    aperp.multiplyScalar(aliForce);
  }
  let newAvec = new THREE.Vector3( avec.x-aperp.x, avec.y-aperp.y, avec.z-aperp.z);
  return newAvec;
}
...
function move_withA_goal( pitch_val) {
  ...
  for (let i = 0; i < count; i++ ) {
    ...
    addition.sub(alignment( aliForce, aforceX, aforceY, aforceZ, addition ));
